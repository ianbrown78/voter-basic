...
<div class="bg-white p-8 rounded-lg shadow-lg max-w-8xl mx-auto">
    ...
    <form action="manage_admins.php" method="POST" class="mb-8 p-6 border border-gray-200 rounded-lg bg-gray-50 space-y-4">
        ...
        <div>
            ...
            <input type="text" name="username" id="username" value="<?php echo htmlspecialchars($admin_to_edit['username'] ?? ''); ?>" <?php echo $admin_to_edit ? 'disabled' : '' ?> required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        </div>
        <div>
